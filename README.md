...
The published coordinates are `com.ondewo:ondewo-s2t-client-java`. Maven:

```xml
<dependency>
  <groupId>com.ondewo</groupId>
  <artifactId>ondewo-s2t-client-java</artifactId>
...
```groovy
dependencies { implementation 'com.ondewo:ondewo-s2t-client-java:VERSION' }
```
...
> [search.maven.org](https://search.maven.org/artifact/com.ondewo/ondewo-s2t-client-java).
...
dependencies { implementation 'com.github.ondewo:ondewo-s2t-client-java:VERSION' }
```

### Jar from the GitHub release

Every [release](https://github.com/ondewo/ondewo-s2t-client-java/releases) carries the built
`ondewo-s2t-client-java-<version>.jar` and its sources jar as assets. Install one into
...
mvn install:install-file \
  -Dfile=ondewo-s2t-client-java-<version>.jar \
  -DpomFile=pom.xml
```
...
```bash
git clone --recurse-submodules https://github.com/ondewo/ondewo-s2t-client-java.git
cd ondewo-s2t-client-java
...
service stubs of this client live in `ondewo.s2t`. Each proto service `Foo` becomes a
`FooGrpc` class with a blocking, a future and an async stub; list what is available with:

```bash
ls src/main/java/ondewo/s2t/*Grpc.java
```

A minimal call, with the bearer token ONDEWO servers expect attached to every request:

```java
import com.ondewo.s2t.auth.BearerToken;
import io.grpc.ManagedChannel;
import io.grpc.ManagedChannelBuilder;
import ondewo.s2t.Speech2TextGrpc;
...
Speech2TextGrpc.Speech2TextBlockingStub stub = new BearerToken(accessToken)
    .attachTo(Speech2TextGrpc.newBlockingStub(channel));
...
the outer class protoc names after the proto file: `ondewo/s2t/speech-to-text.proto` →
`ondewo.s2t.SpeechToText`. No `com.ondewo.s2t` classes are generated, which is why the
...
```
.
├── ondewo-s2t-api/               <----- submodule: the .proto definitions (input)
...
1. `checkout_defined_submodule_versions` — checks out the `ondewo-s2t-api` and
...
```bash
docker run --rm \
  -v $(pwd)/ondewo-s2t-api:/input-volume/ondewo-s2t-api \
  -v $(pwd)/LICENSE:/input-volume/LICENSE \
  -v $(pwd):/output-volume \
  ondewo-java-proto-compiler \
  ondewo-s2t-api ondewo com.ondewo ondewo-s2t-client-java <version>
```
...
* **The output volume is the repository root**, which is precisely the maven project the image
  ...
  is the descriptor being re-rendered from `ONDEWO_S2T_VERSION`, and is intended.
...
`src/main/java/com/ondewo/s2t/auth/`). Two rules follow from the pipeline:
...
| --- | --- |
| `com.ondewo.s2t.stubs.GeneratedMessagesTest` | messages nested in the outer class `ondewo.s2t.SpeechToText` survive a serialize/parse round trip, the `optional string language` of `TranscribeRequestConfig` keeps explicit presence on the wire, every enum starts at its declared zero value, and the proto package in the descriptor is untouched by the `java_package` rewrite |
| `com.ondewo.s2t.stubs.GeneratedServicesTest` | the generated `Speech2TextGrpc` class (found on the compiled classpath, not listed by hand) exposes a usable `ServiceDescriptor`, `TranscribeStream` stays bidirectionally streaming, a unary `ListS2tLanguages` call runs end to end over the in-process transport and the real generated marshallers, and all three stub flavours build against a plain target channel |
| `com.ondewo.s2t.auth.BearerTokenTest` | the hand-written `BearerToken` — validation, header shape, stub immutability |

Coverage is measured with **JaCoCo over the hand-written sources only**
(`com/ondewo/s2t/auth/**`). Generated stubs are machine output and are excluded from the metric,
...
Versions are defined once, in the `Makefile`: `ONDEWO_S2T_VERSION` must match the
`ondewo-s2t-api` release in major and minor version, and is written into the generated `pom.xml` by
the next `make build`.

1. Bump `ONDEWO_S2T_VERSION` and the submodule pins in the `Makefile`.
...
* [ONDEWO S2T API (protos)](https://github.com/ondewo/ondewo-s2t-api)
