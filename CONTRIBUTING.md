...
* **Almost nothing here is hand-written.** `src/main/java` and `pom.xml` are produced by the
  ...
  [ondewo-s2t-api](https://github.com/ondewo/ondewo-s2t-api) (the protos) or in
...
* **Hand-written classes live in their own java package** (for example
  `src/main/java/com/ondewo/s2t/auth/`). The generator deletes only the leaf packages it
...
* **Versions are bumped in the `Makefile`**, never in `pom.xml`: `ONDEWO_S2T_VERSION`
  is written into the descriptor by the next `make build`, and must match the `ondewo-s2t-api` release
