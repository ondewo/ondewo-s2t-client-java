...
# =====================================================================================
# ondewo-s2t-client-java - Makefile
#
# The ONDEWO S2T (Speech-to-Text) gRPC client for java.
#
...
# protos of the ondewo-s2t-api submodule.
#
# Quick start:
...
#
# Versioning: ONDEWO_S2T_VERSION (below) is the single source of truth. It is
...
MAVEN_ARTIFACT_ID=ondewo-s2t-client-java
...
CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| perl -ne 'print if /Release ONDEWO S2T Java Client ${ONDEWO_S2T_VERSION}/../^\*{5}/'`

GH_REPO="https://github.com/ondewo/ondewo-s2t-client-java"
...
install_precommit_hooks: ## Installs pre-commit hooks and sets them up for the ondewo-s2t-client-java repo
...
TEST: ## Prints some important variables
	...
	@echo "Maven Artifact: \t ${MAVEN_GROUP_ID}:${MAVEN_ARTIFACT_ID}:${ONDEWO_S2T_VERSION}"
	@echo "Api Submodule: \t ${ONDEWO_API_DIR} @ ${ONDEWO_S2T_API_GIT_BRANCH}"
...
generate_ondewo_protos: ## Generate java gRPC stubs + the maven project from the proto files
	@test -d ${ONDEWO_PROTOS_DIR} || { echo "$(RED)[ERROR]$(NC) '${ONDEWO_PROTOS_DIR}' is missing - run 'make update_submodules' first"; exit 1; }
	@echo "$(BLUE)[INFO]$(NC) Generating ${MAVEN_GROUP_ID}:${MAVEN_ARTIFACT_ID}:${ONDEWO_S2T_VERSION} from ${ONDEWO_PROTOS_DIR} ..."
	docker run --rm \
...
# flip-flop greps for - is otherwise only noticed by whoever reads the release page afterwards.
	...
	if [ -z "$$notes" ]; then \
		echo "$(RED)[ERROR]$(NC) RELEASE.md has no 'Release ONDEWO S2T Java Client ${ONDEWO_S2T_VERSION}' entry"; \
...
build_gh_release: check_release_notes ## Generate Github Release with CLI and attach the built jars
	...
	gh release create --repo $(GH_REPO) "$(ONDEWO_S2T_VERSION)" \
		-n "$(CURRENT_RELEASE_NOTES)" \
		-t "Release ${ONDEWO_S2T_VERSION}" \
		$(wildcard target/*.jar)
...
# release and is therefore never proven to be resolvable at all.
	...
	base=target/${MAVEN_ARTIFACT_ID}-${ONDEWO_S2T_VERSION}; \
...
# replaced or withdrawn - so the last step stays a deliberate human click.
	...
	@echo "$(GREEN)[SUCCESS]$(NC) uploaded ${MAVEN_GROUP_ID}:${MAVEN_ARTIFACT_ID}:${ONDEWO_S2T_VERSION}"
...
spc: ## Checks if the Release Branch, Tag and pom.xml version already exist
	...
	@if test "$(pom_version)" != "" && test "$(pom_version)" != "${ONDEWO_S2T_VERSION}"; then \
		echo "-- Test 3: pom.xml is at '$(pom_version)', not ${ONDEWO_S2T_VERSION} - run 'make build'!!"; exit 1; \
